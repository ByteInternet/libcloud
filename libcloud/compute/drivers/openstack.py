...
class OpenStack_1_1_NodeDriver(OpenStackNodeDriver):
    ...
    def create_node(
        ...
        :keyword    ex_network_config: The server is launched with a predefined
                                       network config. If this option is used, the
                                       dict is passed through directly and the networks
                                       argument is ignored. Use this in case you need to
                                       provision server instances where the NIC is not
                                       specified by uuid, but by port, fixed_ip or tag.
                                       See https://docs.openstack.org/api-ref/compute/#create-server
        :type       ex_network_config: ``list`` of ``dict`` like:
                                        [{'port': 'network_id'}]
